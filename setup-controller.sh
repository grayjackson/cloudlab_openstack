...
image_id=`openstack image list -f value | grep OL7head | cut -d' ' -f 1`
security_id=`openstack security group list -f value | grep $project_id | cut -d' ' -f 1`
port_id=`openstack port list -f value | grep port1 | cut -d' ' -f 1`
